...
namespace TeduCoreApp.Application.Dapper.Implementation
{
    public class ProductReportService:IProductReportService
    {
        ...
        public async Task<IEnumerable<ProductReportViewModel>> GetProductReport(string fromDate, string toDate)
        {
            ...
            using (var sqlConnection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                try
                {
                    return await sqlConnection.QueryAsync<ProductReportViewModel>(
                        "GetProduct", dynamicParameters, commandType: CommandType.StoredProcedure);
                }
                catch (Exception ex)
                {
                    throw;
